...
if st.session_state.lista_precios:
    ...
    # Lógica de planes y encabezado mejorada
    if "VIRTUS" in d['Modelo']:
        ...
        encabezado = f"**Financiá el 60% de tu unidad en cuotas y adjudicalo con el 40% de su valor.**"
        tp = "Plan 60/40"
        porc = "40%"
        alic_h = int(d['VM'] * 0.4)
    else:
        encabezado = f"**Financiá el 70% de tu unidad en cuotas y adjudicalo con el 30% de su valor.**"
        tp = "Plan 70/30"
        porc = "30%"
        alic_h = int(d['VM'] * 0.3)
    ...
    # Para el Virtus, ocultamos la fila de Alícuota ya
