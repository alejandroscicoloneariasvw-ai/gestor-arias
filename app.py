...
st.set_page_config(page_title="Arias Hnos. | Ventas Pro", layout="wide")
...
with st.sidebar:
    ...
    if modo == "Editar Precios / Manual":
        ...
        if datos_previos and 'Adj_Pactada' in datos_previos: adj_defecto = datos_previos['Adj_Pactada']
            ...
            for l in lineas:
                ...
                if len(p) >= 8:
                    try:
                        ...
                        temp.append({"Modelo": m_n, "VM": int(float(p[1])), "Susc": int(float(p[2])), "C1": int(float(p[3])), "Adh": int(float(p[4])), "C2_13": int(float(p[5])), "CFin": int(float(p[6])), "CPura": int(float(p[7])), "Adj_Pactada": adj_ini})
                    except: continue
...
if st.session_state.lista_precios:
    ...
    msj = (f"Basada en la planilla de *Arias Hnos.* con vigencia al *{st.session_state.fecha_vigencia}*, aquí tienes el detalle de los costos para el:\\n\\n"
           ...
           f"⚠️ **IMPORTANTE:** Al momento de enviarte esto, solo me quedan **2 cupos disponibles** con estas condiciones.\\n\\n"
           f"🎁 Para asegurarte la bonificación del **servicio y el polarizado**, enviame ahora la foto de tu **DNI (frente y dorso)**. Yo reservo el cupo mientras terminás de decidirlo, así no perdés el beneficio por falta de stock. ¿Te parece bien? 📝📲")
